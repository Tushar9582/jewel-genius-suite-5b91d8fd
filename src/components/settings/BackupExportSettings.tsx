  ...
  const handleExport = async (type: ExportType, format: ExportFormat) => {
    ...
    try {
      ...
      if (type === 'all_data' || type === 'system') {
        // Add more data as tables are created
        exportData.exportedAt = new Date().toISOString();
        exportData.exportType = type;
      }

      if (type === 'system' && isAdmin) {
        // Admin-only: get all users' data
      ...
      switch (format) {
        ...
        case 'csv':
          ...
          break;
        case 'pdf':
          // For PDF, we'll create a simple text version (full PDF would need a library)
      ...
      link.href = url;
      link.download = `export-${type}-${format}-${Date.now()}.${extension}`;
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
          ...
          <Badge variant={preferences?.last_backup_at ? 'default' : 'secondary'}>
            {preferences?.last_backup_at ? (
              <><CheckCircle className="w-3 h-3 mr-1" /> Backed Up</>
            ) : (
              <><AlertCircle className="w-3 h-3 mr-1" /> No Backup</>
            )}
          </Badge>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="border rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Export your profile and preferences</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleExport('profile', 'json')}
                disabled={!!exporting}
              >
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleExport('profile', 'pdf')}
                disabled={!!exporting}
              >
          ...
          <div className="border rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium text-sm">All User Data</p>
                <p className="text-xs text-muted-foreground">Export all your data including transactions</p>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleExport('all_data', 'json')}
                disabled={!!exporting}
              >
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => handleExport('all_data', 'csv')}
                disabled={!!exporting}
              >
            ...
            <div className="border border-primary/50 rounded-lg p-4 space-y-3 bg-primary/5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Export all system data (admin only)</p>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button
                  variant="gold"
                  size="sm"
                  onClick={() => handleExport('system', 'json')}
                  disabled={!!exporting}
                >
...
function convertToCSV(data: any): string {
  ...
  if (data.profile) {
    rows.push('--- Profile ---');
    rows.push(Object.keys(data.profile).join(','));
    rows.push(Object.values(data.profile).map(v => `"${v || ''}"`).join(','));
    rows.push('');
  }

  if (data.preferences) {
    rows.push('--- Preferences ---');
    rows.push(Object.keys(data.preferences).join(','));
    rows.push(Object.values(data.preferences).map(v => `"${v || ''}"`).join(','));
    rows.push('');
  }

  if (data.allProfiles) {
    rows.push('--- All Profiles ---');
    if (data.allProfiles.length > 0) {
      rows.push(Object.keys(data.allProfiles[0]).join(','));
      data.allProfiles.forEach((p: any) => {
        rows.push(Object.values(p).map(v => `"${v || ''}"`).join(','));
      });
    }
  }
